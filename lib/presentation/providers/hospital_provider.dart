...
import 'pharmacy_provider.dart'; // 시도/시군구 목록 재사용
...
final hospitalSelectedSidoProvider = StateProvider<String?>((ref) => null);
final hospitalSelectedSgguProvider = StateProvider<String?>((ref) => null);
final hospitalSelectedClCdProvider = StateProvider<String?>((ref) => null); // 종별코드
final hospitalSearchTextProvider = StateProvider<String>((ref) => '');
final hospitalCurrentPageProvider = StateProvider<int>((ref) => 1);

// 병원 목록 검색 Provider
final hospitalListProvider = FutureProvider.family<List<HospitalModel>, HospitalSearchParams>((ref, params) async {
  final apiService = ref.watch(hospitalApiServiceProvider);
  
  // 시군구 코드 대신 시군구명을 사용하도록 수정
  String? sgguName;
  if (params.sgguCd != null) {
    final sgguList = ref.read(hospitalSgguListProvider);
  ...
  return await apiService.getHospitalList(
...
// 병원 검색용 시군구 목록 (시도 선택에 따라 동적으로 변경)
final hospitalSgguListProvider = Provider<List<SgguItem>>((ref) {
...
// 병원 종별 목록
final hospitalTypeListProvider = Provider<List<HospitalTypeItem>>((ref) {
  return [
    HospitalTypeItem(code: null, name: '전체'),
    HospitalTypeItem(code: '01', name: '상급종합병원'),
    HospitalTypeItem(code: '11', name: '종합병원'),
    HospitalTypeItem(code: '21', name: '병원'),
    HospitalTypeItem(code: '31', name: '의원'),
    HospitalTypeItem(code: '41', name: '치과병원'),
    HospitalTypeItem(code: '51', name: '치과의원'),
    HospitalTypeItem(code: '61', name: '조산원'),
    HospitalTypeItem(code: '71', name: '보건소'),
    HospitalTypeItem(code: '72', name: '보건지소'),
    HospitalTypeItem(code: '73', name: '보건진료소'),
    HospitalTypeItem(code: '74', name: '보건의료원'),
    HospitalTypeItem(code: '91', name: '한방병원'),
    HospitalTypeItem(code: '92', name: '한의원'),
  ];
});

class HospitalTypeItem {
  final String? code;
  final String name;

  HospitalTypeItem({required this.code, required this.name});
} 